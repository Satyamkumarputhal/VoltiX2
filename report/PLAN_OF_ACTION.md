...
- **Severity-ceiling finding (found during Phase 3, fixed via V7 migration):** with the originally-seeded `risk_multiplier=1.0` on Zone 1, the anomaly score's hard `Math.min(1.0, ...)` ceiling in `AnomalyDetectionEngine` meant `priorityScore = score × 1.0 ≤ 1.0`, which ALWAYS fell in the LOW severity band — MEDIUM/HIGH/CRITICAL were mathematically unreachable regardless of the model's actual confidence. This was not a bug but a structural consequence of conservative risk calibration (1.0 is the neutral multiplier). Fixed in `V7__Increase_Zone1_Risk_Multiplier_For_Demo.sql` by raising Zone 1 to `risk_multiplier=3.0`, so a typical ONNX score of ~0.64 now produces `priorityScore ≈ 1.92` → MEDIUM severity. This is a **demo-environment tuning choice** to show a realistic range of alert severities, not a claim about real-world risk calibration for this zone. In production, risk_multiplier would be set per-zone based on actual grid topology and incident history.
