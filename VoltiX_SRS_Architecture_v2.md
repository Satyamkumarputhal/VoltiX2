...
5. **Ingestion Performance vs. Durability (fsync/synchronous_commit):** [DECISION NEEDED] Under 1,000 req/s load with standard synchronous PostgreSQL commits (`fsync=on`, `synchronous_commit=on`), write latencies cause massive connection pool exhaustion and a 79.87% request failure rate. Turning these parameters off resolves the bottleneck (100% success rate, p99=15.22ms) but introduces data loss risk on crash. A decision is required on whether to accept this data loss risk in development/testing environments, or design a different ingestion pipeline architecture (e.g. queue/batch writes).
